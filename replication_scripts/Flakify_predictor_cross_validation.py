...
for train_index, test_index in skf.split(input_data, target_data):
    ...
    train_y = torch.tensor(Y_train['which_tests'].values, dtype=torch.long)
    val_y = torch.tensor(y_val['which_tests'].values, dtype=torch.long)
    test_y = torch.tensor(y_test['which_tests'].values, dtype=torch.long)
...
new_row = pd.DataFrame([{'Accuracy': accuracy, 'F1': F1, 'Precision': Precision, 'Recall': Recall, 'TN': TN, 'FP': FP, 'FN': FN, 'TP': TP}])
