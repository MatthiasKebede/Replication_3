...
df.dropna(subset=['final_code', 'flaky'], inplace=True) # # added
...
model_name = "./codebert-base" # # changed
...
def give_test_data_in_chunks(x_test):
    ...
    for g, x_test_chunk in x_test.groupby(np.arange(len(x_test)) // n):
        tokens_test = tokenizer(
            x_test_chunk.tolist(), max_length=500, padding='max_length', truncation=True)
        test_seq = torch.tensor(tokens_test['input_ids'])
        test_mask = torch.tensor(tokens_test['attention_mask'])
...
for i in project_name:
    ...
    # converting labels of train, validation and test into tensors
    train_y = torch.tensor(Y_train['flaky'].values, dtype=torch.long)
    val_y = torch.tensor(y_val['flaky'].values, dtype=torch.long)
    test_y = torch.tensor(y_test['flaky'].values, dtype=torch.long)
    ...
    new_row = pd.DataFrame([{'project_name': project_Name, 'Accuracy': accuracy, 'F1': F1, 'Precision': Precision, 'Recall': Recall, 'TN': TN, 'FP': FP, 'FN': FN, 'TP': TP}])
